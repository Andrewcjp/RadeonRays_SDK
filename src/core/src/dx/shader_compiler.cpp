...
namespace rt::dx
{
	...
	static std::vector<Entry> s_isnt;
	static void WriteFile()
	{
		std::ofstream ofs("../../src/core/src/dx/mapped_kernels.h"/*, std::ios_base::app*/);
		std::string top = "#include <vector>\n\
#include <string>\n\
#include <map>\n\
#include <stdint.h>\n\
namespace rt::dx::shaders\n\
		{\n\
			std::vector<uint32_t> const& GetShaderCode(std::string const& key)\n\
			{\n\
			static std::map<std::string, std::vector<uint32_t>> shaders = {\n	";
		ofs << top << std::endl;
		for (int idx = 0; idx < s_isnt.size(); idx++)
		{
			std::string key = s_isnt[idx].Key;
			ofs << "{ " << "\"" << key << "\", " << '{' << std::endl;
			for (size_t i = 0; i < s_isnt[idx].code.size(); i++)
			{
				ofs << s_isnt[idx].code[i];
				if (i < s_isnt[idx].code.size() - 1)
				{
					ofs << ", ";
				}
			}
		...
		ofs << "};" << std::endl;
		std::string End = "    return shaders.at(key);\n\
	}\n\
} // rt::dx::shaders\n";
		ofs << End << std::endl;
		ofs.close();
		...
		const std::vector<std::string>& defines)
	{
...
#if !defined(WRITE_MAPPED_HEADER) && defined(RR_EMBEDDED_KERNELS)
		try
		{
		...
		ofs << "}}," << std::endl;
#endif
#ifdef WRITE_MAPPED_HEADER
		Entry e;
		e.Key = key;
		uint32_t *charBuffer = (uint32_t*)blob->GetBufferPointer();
		e.code = std::vector<uint32_t>(charBuffer, charBuffer + blob->GetBufferSize() / sizeof(uint32_t));
		s_isnt.push_back(e);
		WriteFile();
#endif
		return Shader{ blob };
